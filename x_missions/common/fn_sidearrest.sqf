...
if (_docreateinf) then {
	["specops", (floor (random 4)) min 2, "allmen", (floor (random 4)) min 2, d_x_sm_pos select 0, 300, true] spawn d_fnc_CreateInf;
	sleep 2.333;
};
if (_docreatearmor) then {
	[selectRandom ["aa", "tank"], 1, selectRandom ["tracked_apc", "wheeled_apc"], 2, selectRandom ["jeep_mg", "jeep_gl"], 2, d_x_sm_pos select 0, 1, 400, true] spawn d_fnc_CreateArmor;
	sleep 2.333;
	["stat_mg", 1, "stat_gl", 1, "", 0, d_x_sm_pos select 0, 1, 100, false] spawn d_fnc_CreateArmor;
	sleep 1;
};
	...
	if (!_rescued) then {
		...
		if !(_nobjs isEqualTo []) then {
			if (!d_no_ai || {d_tt_ver}) then {
				...
				{
					if ((str _x) select [0, 8] == "d_artop_") exitWith {
						_rescued = true;
						_officer enableAI "PATH";
						[_officer] join _x;
						[_officer, true] remoteExecCall ["setCaptive", _officer];
