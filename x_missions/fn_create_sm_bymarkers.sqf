...
// example, number must be unique, otherwise it will overwrite the previous entry
...
private _infhelper_fnc = {
	...
	["specops", parseNumber (_marar select (_fidx + 1)), "allmen", parseNumber (_marar select (_fidx + 2)), markerPos _curmar, parseNumber (_marar select (_fidx + 3)), _marar select (_fidx + 4) == "true"]
};

private _armorhelper_fnc = {
	params ["_fidx", "_marar", "_curmar"];
	["aa", parseNumber (_marar select (_fidx + 1)), "tracked_apc", parseNumber (_marar select (_fidx + 2)), "tank", parseNumber (_marar select (_fidx + 3)), markerPos _curmar, parseNumber (_marar select (_fidx + 4)), parseNumber (_marar select (_fidx + 5)), _marar select (_fidx + 6) == "true"]
};
...

{
	...
	for "_i" from 3 to (count _marar - 1) do {
		private _str = toLower (_marar select _i);
	...
	if (call d_fnc_checkSHC) then {
		...
		if (_fidx != -1) then {
			if (_marar select (_fidx + 1) == "no") then {
				_onesmar set [3, false];
			} else {
				(_onesmar select 3) pushBack ([_fidx, _marar, _curmar] call _armorhelper_fnc);
			};
		};
		...
		if (_fidx != -1) then {
			if (_marar select (_fidx + 1) == "no") then {
				...
				(_onesmar select 4) pushBack ([_fidx, _marar, _curmar] call _infhelper_fnc);
			};
		};
	};
	...
	call {
		if (_smtype == "convoy") exitWith {
			call {
				if (_subtype == "start") exitWith {
					(_onesmar select 2) set [0, markerPos _curmar];
				...
				if (_subtype == "end") exitWith {
					(_onesmar select 2) set [1, markerPos _curmar];
				};
			};
		};
		...
		if (_smtype == "stealflag") exitWith {
			call {
				if (_subtype == "flag") exitWith {
					(_onesmar select 2) pushBack (markerPos _curmar);
					if (hasInterface && {_onesmar select 5 == ""}) then {
						_onesmar set [5, "775"];
						_onesmar set [6, "776"];
					};
				};
		...
		if (_smtype == "tankdepot") exitWith {
			call {
				if (_subtype == "tank") exitWith {
					(_onesmar select 2) pushBack [markerPos _curmar, markerDir _curmar];
					if (hasInterface && {_onesmar select 5 == ""}) then {
						_onesmar set [5, "1810"];
						_onesmar set [6, "767"];
					};
		...
		if (_smtype == "deliver") exitWith {
			call {
				if (_subtype == "") exitWith {
					(_onesmar select 2) pushBack (markerPos "d_sm_loga");
					(_onesmar select 2) pushBack (markerPos _curmar);
					_onesmar set [7, markerDir _curmar];
					if (hasInterface && {_onesmar select 5 == ""}) then {
						_onesmar set [5, "1664"];
		...
		if (_smtype == "evac") exitWith {
			call {
				if ((_onesmar select 2) isEqualTo []) then {
					(_onesmar select 2) pushBack (markerPos _curmar);
					_onesmar set [7, markerDir _curmar];
					if (hasInterface && {_onesmar select 5 == ""}) then {
						_onesmar set [5, "1803"];
				...
				if (_subtype == "time") then {
					if (d_fnc_checkSHC) then {
						_onesmar pushBack (parseNumber (_marar select (_subtypeidx + 1)))
					};
				};
			};
		};
