			...
			if (_distu <= 200) then {
				...
				if !(_targetPos isEqualTo []) then {
					...
					if (_dodraw) then {
						...
						drawIcon3D [_rtex, [d_pnhudothercolor, d_pnhudgroupcolor] select (group _u == _grpp), _targetPos, 0.4, 0.4, 0, _tex, 1, __d_textsize_dr3d, "RobotoCondensed"]; // PuristaSemibold PuristaMedium
					};
				};
			};
		};
