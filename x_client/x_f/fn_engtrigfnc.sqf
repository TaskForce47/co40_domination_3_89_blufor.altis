	...
	d_actionID2 = [
		...
		/* 7 code executed per tick */		{
			...
			if (_damage > 0) then {
				...
				[d_orig_sfunc_obj, _damage] remoteExecCall ["setDamage"];
			};
		...
		/* 8 code executed on completion */	{
			...
			if (d_with_ranked) then {
				private _addscore = if (d_orig_sfunc_obj isKindOf "Air") then {
					(d_ranked_a select 1) select 0
				} else {
					if (d_orig_sfunc_obj isKindOf "Tank") then {
						(d_ranked_a select 1) select 1
					} else {
						if (d_orig_sfunc_obj isKindOf "Car") then {
							(d_ranked_a select 1) select 2
						} else {
							(d_ranked_a select 1) select 3
						};
					};
				};
				if (_addscore > 0) then {
