...
//if (isNil "d_textsize_dr3d") then {
//	d_textsize_dr3d = 1 / 30;
//};
				...
				if (_distu <= d_dist_pname_hud) then {
					...
					if !(_targetPos isEqualTo []) then {
						...
						if (_dodraw) then {
							...
							drawIcon3D [_rtex, if (group _u == _grpp) then {d_pnhudgroupcolor} else {d_pnhudothercolor}, _targetPos, 0.4, 0.4, 0, _tex, 1, __d_textsize_dr3d, "RobotoCondensed"]; //PuristaSemibold PuristaMedium
						};
					};
				};
			};
